...
class HexView(QTableView):
    ...
    def __init__(self, parent=None):
        ...
        self._column_byte_pos = -1  # Byte position for column selection
    ...
    def _on_selection_changed(self, selected, deselected):
        ...
        if self._selection_mode == self.SELECTION_ROW:
            ...
            min_col, max_cols = min(cols), max(cols)

            # For block selection, we need to track start/end positions
            if self._block_start_row >= 0 and self._block_start_col >= 0:
                start_row = min(self._block_start_row, max_row)
                end_row = max(self._block_start_row, max_row)
                start_col = min(self._block_start_col, max_cols)
                end_col = max(self._block_start_col, max_cols)

                ranges = []
                for row in range(start_row, end_row + 1):
                    start = row * bytes_per_row + start_col
                    end = row * bytes_per_row + end_col
                    if start < file_size:
            ...
            # Continuous selection: standard behavior
            # Find min and max offsets from selected indexes
            min_offset = float('inf')
            max_offset = 0

            for idx in indexes:
                row = idx.row()
                # Simple calculation - continuous bytes
                offset = row * bytes_per_row
                if offset < min_offset:
                    min_offset = offset
                if offset + bytes_per_row > max_offset:
                    max_offset = offset + bytes_per_row

            max_offset = min(max_offset - 1, file_size - 1)

            if min_offset <= max_offset:
                self._model.set_selection(min_offset, max_offset)
                self.selection_changed.emit(min_offset, max_offset)
            else:
                self._model.clear_selection_highlight()
                self.selection_changed.emit(-1, -1)
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if index.isValid():
                if self._selection_mode == self.SELECTION_BLOCK:
                    # Start block selection - store start position
    ...
    def mouseMoveEvent(self, event):
        """Handle mouse move for block selection."""
    ...
    def mouseReleaseEvent(self, event):
        """Handle mouse release for block selection."""
        if self._selection_mode == self.SELECTION_BLOCK and event.button() == Qt.MouseButton.LeftButton:
            self._block_start_row = -1
            self._block_start_col = -1
    ...
    def _do_column_selection(self, pos):
        ...
        # Offset area is about 10 character widths
        offset_chars = 10
        char_width = col_width / (offset_chars + bytes_per_row * 3)
        x_offset = pos.x()

        byte_pos = int((x_offset / char_width - offset_chars) / 3)
        # Clamp to valid range
        byte_pos = max(0, min(byte_pos, bytes_per_row - 1))
